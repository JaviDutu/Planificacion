...
# Capacity used for the transporters in this lab.
...
def main():
    ...
    if options.drones < 1:
        print("Must have at least 1 drone")
        sys.exit(1)

    if options.carriers < 1:
        print("Must have at least 1 carrier for this part")
        sys.exit(1)
    ...
    # Determine the goal conditions for each person.
    need = setup_person_needs(options, crates_with_contents)

    # Assign each person to a random non-depot location.
    person_locations = []
    for x in range(options.persons):
    ...
    # Open the file and start writing the PDDL problem.
    ...
    # Drones start at the depot, empty-handed and available.
    for x in drone:
        f.write("\t(at-drone " + x + " depot)\n")
        f.write("\t(hand-empty " + x + ")\n")
        f.write("\t(drone-free " + x + ")\n")

    # Carriers start at the depot, with full capacity and available.
    for x in carrier:
        f.write("\t(at-transporter " + x + " depot)\n")
        f.write("\t(free-slots " + x + " n" + str(TRANSPORTER_CAPACITY) + ")\n")
        f.write("\t(transporter-free " + x + ")\n")

    # All crates start at the depot.
    for x in crate:
        f.write("\t(at-crate " + x + " depot)\n")

    # Persons are placed randomly in non-depot locations and start available.
    ...
    # Successor relation for numeric capacity representation.
    for i in range(TRANSPORTER_CAPACITY):
        f.write("\t(siguiente n" + str(i) + " n" + str(i + 1) + ")\n")

    # Initialize flight times between every pair of locations.
    ...
    for x in range(options.persons):
        for y in range(len(content_types)):
            if need[x][y]:
                f.write("\t(has-content " + person[x] + " " + content_types[y] + ")\n")
    f.write("\t))\n")

    ####################################################################################
    # Metric
    ####################################################################################
    # For temporal planning in part 3 we minimize total-time.
