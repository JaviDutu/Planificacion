...
#
# Adapted for Parte 2: transporters + action costs
########################################################################################
...
# Capacity used in Parte 2 benchmarks
...
# use them implicitly by using *distances* in order to calculate flight costs.
...
# locations supplied by their integer indexes.
...
# Randomly distribute crate contents.
...
# need[personid][contentid] is true iff that person needs that content.
...
def main():
    ...
    # Determine which content each person needs
    ...
    with open(problem_name + ".pddl", 'w') as f:
        ...
        # Drones start at depot with empty hand
        for x in drone:
            f.write("\t(at-drone " + x + " depot)\n")
            f.write("\t(hand-empty " + x + ")\n")

        # Transporters start at depot, empty, with full capacity
        for x in carrier:
        ...
        # People start at random non-depot locations
        ...
        # Numeric chain for transporter capacity
